/** CppLinuxSerial Example 0A
 * Sends a single hello message from serial device A 
 * and reads a single hello mesage from serial device B.
 * Requires the user to input the device number 
 * and desired baud rate
 * 
 * build: (examples folder)
 mkdir -p output build
 g++ serialExample0A.cpp -lCppLinuxSerial -o ../output/example0A
 *
 * run: (output folder)
 ./example0A
 * 
